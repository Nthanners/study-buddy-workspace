...
function makeApi(token, onUnauth) {
  ...
  return {
    get:  (p)    => fetch(p, { headers: auth }).then(handle),
    post: (p, b) => fetch(p, { method: 'POST',   headers: json, body: JSON.stringify(b) }).then(handle),
    put:  (p, b) => fetch(p, { method: 'PUT',    headers: json, body: JSON.stringify(b) }).then(handle),
    del:  (p)    => fetch(p, { method: 'DELETE', headers: auth }).then(handle),
  }
}
  ...
  return (
    <div className="waifu-widget" aria-hidden="true">
      <div className="waifu-float">
        <img src="/waifu.png" alt="" className="waifu-img" draggable="false" />
      </div>
    </div>
  )
}
